<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ,<link rel="stylesheet" href=".//profile.css">
    <title>name project</title>
</head>
<header>
    <div id="Header_Home">
        <img src="../../assets/logo-ingressofit.jpeg" alt="logo_site" />
        <nav>
            <ul>
                <a href="/projetoADS">HOME</a>
                <a href="/projetoADS/pages/login/login.php">LOGIN</a>
                <a href="/projetoADS/pages/cadastro/cadastro.php">CADASTRAR</a>
                <a href="/projetoADS/pages/contato/contato.php">CONTATO</a>
            </ul>
        </nav>
    </div>
</header>
<body>
    <div id="Profile_Conteiner">
        <h1>Área do Cliente</h1>
        <br/>
        <div id="Info_pessoais">
            <label>Nome:</label>
            <p>Joao Victor de Andrade Micheletti</p>
            <label>E-mail:</label>
            <p>jv604014@gmail.com</p>
            <label>Telefone:</label>
            <p>1193222-3533</p>
            <a href="/projetoADS/index.php">Log-out</a>
        </div>
        <hr/>
        <div id="Ingressos">
            <h2>Ingressos comprados</h2>
            <br/>
            <div id="Conteudo_Shows">
                <br/>
                <div id="Evento">
                    <img src="../../assets/home/gus.jpeg" alt="img_show">
                    <div id="Info_show">
                        <p>Gustavo Lima</p>
                        <p>19:30 - 02/02/2024</p>
                        <p>Santo Amaro</p>
                        <p>R$ 150,00</p>
                    </div>
                </div> 
                <div id="Evento">
                    <img src="../../assets/home/leonardo.jpeg" alt="img_show">
                    <div id="Info_show">
                        <p>Leonardo</p>
                        <p>20:00 - 01/01/2024</p>
                        <p>Santos</p>
                        <p>R$ 200,00</p>
                    </div>
                </div> 
            </div> 
        </div>
        </div>
    </div>
    <footer id="Footer">
        <ul><img src="../../assets/logo-ingressofit.jpeg" style="width: 200px;" alt="logo"></ul>
        <ul>
            <h5>Sobre</h5>
            <br/>
            <p>Quem Somos</p>
            <p>Dúvidas</p>
            <p>Seja nosso Parceiro</p>
            <p>Trabalhe conosco</p>
            <p>Termos de Uso</p>
        </ul>
        <ul>
            <h5>Fale conosco</h5>
            <br/>
            <p>11 4002-8922</p>
            <p>Contato@gmail.com</p>
        </ul>
    </footer>
    <div id="Down">
        <br/>
        <p>2023 © Todos os Direitos Reservados | IngressoFIT</p>
        <br/>
    </div>
</body>
</html>