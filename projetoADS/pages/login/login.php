<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    ,<link rel="stylesheet" href="./style.css">
    <title>name project</title>
</head>
<header>
    <div id="Header_Home">
        <img src="../../assets/logo-ingressofit.jpeg" alt="logo_site" />
        <nav>
            <ul>
                <a href="/projetoADS">HOME</a>
                <a href="/projetoADS/pages/login/login.php">LOGIN</a>
                <a href="/projetoADS/pages/cadastro/cadastro.php">CADASTRAR</a>
                <a href="/projetoADS/pages/contato/contato.php">CONTATO</a>
            </ul>
        </nav>
    </div>
</header>
<body>
    <div id="Login_conteiner">
        <br/>
        <h1>Área do Cliente</h1>
        <br/>
        <form id="Form_Login">
            <p>Para prosseguir com sua compra de ingresso é necessário efetuar o login no sistema.</p>
            <br/>
            <p id="log">Login</p>
            <br/>
            <label>
                Email:
                <input id="CX" type="email" placeholder="Email"/>
            </label>
            <br/>
            <label>
                Senha:
                <input id="CX" type="password" placeholder="Senha"/>
            </label>
            <br/>
            <button id="BTN">Login</button>
        </form>
    </div>
    <footer id="Footer">
        <ul><img src="../../assets/logo-ingressofit.jpeg" style="width: 200px;" alt="logo"></ul>
        <ul>
            <h5>Sobre</h5>
            <br/>
            <p>Quem Somos</p>
            <p>Dúvidas</p>
            <p>Seja nosso Parceiro</p>
            <p>Trabalhe conosco</p>
            <p>Termos de Uso</p>
        </ul>
        <ul>
            <h5>Fale conosco</h5>
            <br/>
            <p>11 4002-8922</p>
            <p>Contato@gmail.com</p>
        </ul>
    </footer>
    <div id="Down">
        <br/>
        <p>2023 © Todos os Direitos Reservados | IngressoFIT</p>
        <br/>
    </div>
</body>
</html>